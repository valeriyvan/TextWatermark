...
-(NSString*)	uniqueFileName: (NSString*)oldName
{
	...
	{
		if( [suffix length] == 0 )  // Build "/folder/file1"-style path: // комментарий отредактировал для соответствия следующий строке
			fname = [baseName stringByAppendingString: [NSString stringWithFormat:@"%i", n++]]; // убрал здесь пробел в имени файла
		else						// Build "/folder/file1.suffix"-style path: // комментарий отредактировал для соответствия следующий строке
			fname = [baseName stringByAppendingString: [NSString stringWithFormat:@"%i.%@", n++, suffix]]; // убрал здесь пробел в имени файла
